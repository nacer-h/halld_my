...
{
  // TFile *fsim = new TFile(Form("/u/ahamdi/halld_my/pippimkpkm/pippimkpkm__B4_sim_recon17_1v03_%scut.root",cut.Data()));
  TFile *fsim = new TFile("/data.local/nacer/halld_my/pippimkpkm/input/phifo_genr8_17v3_chi100.root");
  TFile *fdata = new TFile("/data.local/nacer/halld_my/pippimkpkm/input/pippimkpkm_17v21_chi100.root");
  ...
  TString cutlist = "&& kpkm_mf>1.005 && kpkm_mf<1.035 && (abs(kp_dttof)<0.135 || kp_dttof == -999) && (abs(kp_dtbcal)<0.4125 || kp_dtbcal == -999) && (abs(kp_dtfcal)<1 || kp_dtfcal == -999) && (abs(pip_dttof)<0.15 || pip_dttof == -999) && (abs(pip_dtbcal)<0.85 || pip_dtbcal == -999) && (abs(p_dttof)<0.27 || p_dttof == -999) && abs(mm2)<0.01 && kp_x4_kin.Z()<80 && kp_x4_kin.Z()>50 &&"; //  (abs(kp_dtfcal)<1 || kp_dtfcal == -999) && (abs(pip_dttof)<0.125 || pip_dttof == -999) && p_x4_kin.Z()<80 && p_x4_kin.Z()>50 && beam_p4_kin.E()>7.95 &&

  double cutsmin = 0; //= hdata_postcut->GetXaxis()->GetBinLowEdge(1);
  double cutsmax = 5;//= hdata_postcut->GetXaxis()->GetBinUpEdge(600);
  ...
  for (int j = 0; j < 20; j++)
  {
    ...
    tdata->Project(Form("hdata_YMass_%d",j), "kpkmpippim_mf", Form("w8*(kpkmpippim_uni"+cutlist+"-t_kin<%f)",cuts[j])); //cuts[j] -t_kin<1 && beam_p4_kin.E()>6 && pt<0.4
    ...
    hdata_YMass[j]->SetTitle(Form("-t < %.2f",cuts[j]));
 ...
 // *********************************************** Sim  ***********************************************
  ...
  for (int j = 0; j < 20; j++)
  {
    ...
    tsim->Project(Form("hsim_YMass_%d",j), "kpkmpippim_mf", Form("w8*(kpkmpippim_uni"+cutlist+"-t_kin<%f)",cuts[j]));
    // hsim_YMass->Rebin(4);
    hsim_YMass[j]->SetMarkerStyle(20);
    hsim_YMass[j]->SetMarkerSize(0.7);
    cout << "hsim_YMass = " << hsim_YMass[j] << endl;
    hsim_YMass[j]->SetTitle(Form("-t < %.2f",cuts[j]));
    hsim_YMass[j]->Draw("e");
...
//   {
   for (int j = 0; j < 20; j++)
   {
    ...
    tdata->Project(Form("hdata_PhiMass_%d",j), "kpkm_mf", Form("w8*(kpkm_uni"+cutlist+"(abs(p_dttof)<%f || p_dttof == -999))",cuts[j]));
    ...
    hdata_PhiMass[j]->SetTitle(Form("|#Delta T| < %.3f",cuts[j]));
...
//   {
   for (int j = 0; j < 20; j++)
   {
    ...
    tdata->Project("hdata_PhiMass", "kpkm_mf", Form("w8*(kpkm_uni && kin_chisq<60 && abs(mm2)<0.03 && sqrt((p_x4_kin.X()*p_x4_kin.X())+(p_x4_kin.Y()*p_x4_kin.Y()))<%f)", cuts[j]));
    //(pip_dttof<-990 || abs(pip_dttof)<0.5) && (pip_dtbcal<-990 || abs(pip_dtbcal)<1.0) && (pip_dtfcal<-990 || abs(pip_dtfcal)<0.5) && (kp_dttof<-990 || abs(kp_dttof)<0.3) && (kp_dtbcal<-990 || abs(kp_dtbcal)<0.6) && (kp_dtfcal<-990 || abs(kp_dtfcal)<2.375) && (p_dttof<-990 || abs(p_dttof)<0.48) && (p_dtbcal<-990 || abs(p_dtbcal)<%f) && (p_dtfcal<-990 || abs(p_dtfcal)<1.9))",cuts[j]));
    // hdata_PhiMass->Rebin(4);
    hdata_PhiMass->SetMarkerStyle(20);
    hdata_PhiMass->SetMarkerSize(0.7);
    cout << "hdata_PhiMass = " << hdata_PhiMass << endl;
    hdata_PhiMass->SetTitle(Form("|#Delta T| < %.2f",cuts[j]));
    hdata_PhiMass->Draw("e");
